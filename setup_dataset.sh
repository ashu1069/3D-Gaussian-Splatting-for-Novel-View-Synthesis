...
if python download_mipnerf360.py --scene ${SCENE} --download_dir ${DOWNLOAD_DIR}; then
    ...
    echo "   python prepare_mipnerf360.py --input_dir ${DOWNLOAD_DIR}/${SCENE} --output_dir ${PREPARED_DIR} --scene_name ${SCENE}"
    echo ""
    exit 1
fi
...
if python prepare_mipnerf360.py \
    ...
    echo "  python train.py --data_dir ${PREPARED_DIR} --output_dir output/${SCENE} --pcd_path ${PREPARED_DIR}/pointcloud.ply"
    echo ""
else
    echo ""
    echo "=========================================="
